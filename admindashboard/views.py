...
class SuperAdminEditEventView(APIView):
    permission_classes = [AllowAny]
    ...
    def put(self, request, id):
        try:
            ...
            event.save()
            return Response({"status": True, "message": "Event updated successfully", "id": event.id})
        except Event.DoesNotExist:
            return Response({"status": False, "message": "Event not found"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({"status": False, "message": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


#  Delete Event
class SuperAdminDeleteEventView(APIView):
    permission_classes = [AllowAny]
...
class SuperAdminDeleteEventCommentView(APIView):
    permission_classes = [AllowAny]
...
#  Delete Event Media
class SuperAdminDeleteEventMediaView(APIView):
    permission_classes = [AllowAny]

    def delete(self, request, id):
        try:
            media = EventMedia.objects.get(id=id)
            media.delete()
